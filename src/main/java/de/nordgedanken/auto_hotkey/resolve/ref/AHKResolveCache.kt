...
import de.nordgedanken.auto_hotkey.psi.AHKPsiChangeListener
import de.nordgedanken.auto_hotkey.psi.AHKStructureChangeListener
import de.nordgedanken.auto_hotkey.psi.AHK_PSI_CHANGE_TOPIC
import de.nordgedanken.auto_hotkey.psi.AHK_STRUCTURE_CHANGE_TOPIC
...
class RsResolveCache(project: Project): Disposable {
    ...
    private val ahkStructureDependentCache: ConcurrentMap<PsiElement, Any?>
        get() = _ahkStructureDependentCache.getOrCreateMap()
    ...
    init {
        ...
        connection.subscribe(AHK_STRUCTURE_CHANGE_TOPIC, object : AHKStructureChangeListener {
            override fun ahkStructureChanged(file: PsiFile?, changedElement: PsiElement?) =
                    onAHKStructureChanged(file)
        })
        ...
        connection.subscribe(AHK_PSI_CHANGE_TOPIC, object : AHKPsiChangeListener {
            override fun ahkPsiChanged(file: PsiFile, element: PsiElement, isStructureModification: Boolean) =
                    onAHKPsiChanged(element)
        })
    }
    ...
    private fun refineDependency(key: PsiElement, dep: ResolveCacheDependency): ResolveCacheDependency =
            when (key.containingFile.virtualFile) {
                ...
                // handle evens from outer PSI. For example, AHK language is injected to Kotlin's string
    ...
    private fun onAHKStructureChanged(file: PsiFile?) {
        _ahkStructureDependentCache.set(null)
    ...
    private fun onAHKPsiChanged(element: PsiElement) {
        // 1. The global resolve cache invalidates only on AHK structure changes.
        ...
        val referenceElement = element.parent as? AHKReferenceElement ?: return
        val referenceNameElement = referenceElement.referenceNameElement ?: return
        if (referenceNameElement == element) {
            /*referenceElement.ancestors.filter { it is AHKReferenceElement }.forEach {
                ahkStructureDependentCache.remove(it)
            }*/
        }
    }
...
enum class ResolveCacheDependency {
    /**
     * Depends on the nearest [AHKModificationTrackerOwner] and falls back to
     * [AHKPsiManager.ahkStructureModificationTracker] if the tracker owner is not found.
     *
    ...
    /**
     * Depends on [AHKPsiManager.ahkStructureModificationTracker]
     */
    AHK_STRUCTURE,

    /**
     * Depends on both [LOCAL] and [AHK_STRUCTURE]. It is not the same as "any PSI change", because,
     * for example, local changes from other functions will not invalidate the value
     */
    LOCAL_AND_AHK_STRUCTURE,
