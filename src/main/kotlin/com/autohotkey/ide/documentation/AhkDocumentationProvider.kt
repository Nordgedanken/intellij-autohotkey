...
class AhkDocumentationProvider : DocumentationProvider, ExternalDocumentationHandler {
    ...
    private fun generateAhkDocumentationReferenceUrlFor(element: PsiElement?): String? {
        ...
        val pathInChm =
            ChmArchiveUtil.getPathInChm(chm, element.text)?.removePrefix("docs/")
                ?: return null
    ...
    override fun generateDoc(
        ...
        val pathInChm =
            ChmArchiveUtil.getPathInChm(chm, element.text)
                ?: return null

        return extractHtmlText(chm, pathInChm)
    }

    private fun extractVariableText(
        chm: ArchiveHandler,
        variableName: String,
    ): String? =
        runCatching {
            val variablesPage =
                ChmArchiveUtil.getPathInChm(chm, "Variables")
                    ?: return null
    ...
    override fun fetchExternalDocumentation(
        ...
        val pathInChm =
            ChmArchiveUtil.getPathInChm(chm, page)
